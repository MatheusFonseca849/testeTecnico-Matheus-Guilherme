...
export const exercicio3 = (): number[] => {
  ...
  const filteredSales = dados.filter(
    (dado: DadoFaturamento) => dado.valor > 0.0
  );
  ...
  // Calcula a média
  const sum: number = filteredSales.reduce(
    (a: number, b: DadoFaturamento) => a + b.valor,
    0
  );
...
export const exercicio4 = (): StateData[] => {
  ...
  const total: number = stateInvoiceArray.reduce(
    (a: number, b: StateData) => a + b.invoice,
    0
  );
